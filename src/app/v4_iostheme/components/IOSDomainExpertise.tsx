...
const IOSDomainExpertise: React.FC<DomainExpertiseProps> = ({ className = '' }) => {
    const { domainExpertise } = portfolioData.personalInfo;
    if (!domainExpertise || domainExpertise.length === 0) return null;
    ...
    return (
        <div className={`${className} w-full overflow-hidden relative group`}>
            <style>{`
        @keyframes domain-marquee { 0% { transform: translateX(0); } 100% { transform: translateX(-33.33%); } }
        .animate-domain-marquee { animation: domain-marquee 40s linear infinite; }
        .group:hover .animate-domain-marquee { animation-play-state: paused; }
      `}</style>
            ...
            <div className="flex gap-4 w-max animate-domain-marquee pl-4">
                {marqueeItems.map((domain, index) => (
                    <div
                        key={index}
                        className="flex items-center gap-2 bg-white/5 px-5 py-3 rounded-full border border-white/10 shadow-sm whitespace-nowrap text-gray-300 hover:text-white hover:bg-white/10 hover:border-blue-500/50 transition-all duration-300 group/item cursor-default backdrop-blur-md"
                    >
...
export default IOSDomainExpertise;
